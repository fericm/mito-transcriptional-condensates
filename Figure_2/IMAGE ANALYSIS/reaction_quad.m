function [P,DATA,I,xval] = reaction_quad(im,z,T,leg,cr,cg,cb,cfr,win)

%Open the images

str_r=strcat(im,'z',sprintf('%03d',z),'_c',sprintf('%03d',cr),'.tif') %red - 561 4
if exist(str_r)~=0
    ...
    imagesc(Image_r)
    if nargin==9
        Image_r=Image_r(win,win);
    end
else
    Image_r=zeros(1942,1942);
end

str_g=strcat(im,'z',sprintf('%03d',z),'_c',sprintf('%03d',cg),'.tif'); %green - 488 3
if exist(str_g)~=0
...
str_b=strcat(im,'z',sprintf('%03d',z),'_c',sprintf('%03d',cb),'.tif'); %blue - 405 1
if exist(str_b)~=0
    ...
    imagesc(Image_b)
    if nargin==9
        Image_b=Image_b(win,win);
    end
else
    Image_b=zeros(1942,1942);
end

str_fr=strcat(im,'z',sprintf('%03d',z),'_c',sprintf('%03d',cfr),'.tif'); %far-red 640 2
if exist(str_fr)~=0
